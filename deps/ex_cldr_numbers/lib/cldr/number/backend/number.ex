    ...
    quote location: :keep, bind_quoted: [backend: backend, config: Macro.escape(config)] do
      defmodule Number do
        ...
        * `currency`: is the currency for which the number is formatted. If `currency`
          is set and no `:format` is set, `:format` will be set to either the
          `:default_currency_format` key of the backend configuration or the currency
          format derived from the local. Currency may be any
          ...
          identifier (crypto currency) or `:from_locale` in which case the currency
        ...
        * `:wrapper` is a 2-arity function that will be called for each number component
        ...
        `#{inspect __MODULE__}.to_string/2`. These options are:

        * `cu`: defines the currency to be used if the `:currency` option to `#{inspect __MODULE__}.to_string/2`
          is set to `:from_locale`

        * `nu`: defines the number system to be used if the `:number_system` option to
          `#{inspect __MODULE__}.to_string/2` is not specified.

        * `cf`: defines the currency format to be used by `#{inspect __MODULE__}.to_string/2` if no `:format`
          is specified, a `:currency` option is specified and the backend configuration key
          `:default_currency_format` is not specified. That is, the priority for determining which
          currency format to use is:
        ...
        applied to the symbol than the number.  For example:
            ...
            ...>   string, :literal -> "<span>" <> string <> "</span>"
        ...
        correctly encoded.  For example:
            ...
            ...>   string, :literal -> Phoenix.HTML.Tag.content_tag(:span, string)
            ...>   string, _other -> string
            ...> end)
            {:ok, "<span class=\\"symbol\\">$</span><span class=\\"number\\">100.00</span>"}
