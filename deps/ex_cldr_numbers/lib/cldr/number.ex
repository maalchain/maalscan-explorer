defmodule Cldr.Number do
  ...
  * `cu`: defines the currency to be used if the `:currency` option to `Cldr.Number.to_string/2`
    is set to `:from_locale`

  * `nu`: defines the number system to be used if the `:number_system` option to
    `Cldr.Number.to_string/2` is not specified.

  * `cf`: defines the currency format to be used by `Cldr.Number.to_string/2` if no `:format`
