defmodule BlockScoutWeb.API.V1.HealthController do
  ...
  def error({:error, number, timestamp}) do
    ...
    %{
      ...
      "error_description" =>
        "There are no new blocks in the DB for the last #{healthy_blocks_period_formatted} mins. Check the healthiness of Ethereum archive node or the Maalscan DB instance",
      "data" => %{
        "latest_block_number" => to_string(number),
        "latest_block_inserted_at" => to_string(timestamp)
      }
