defmodule Indexer.PendingTransactionsSanitizer do
    ...
    |> Enum.each(fn {pending_tx, ind} ->
      ...
      with {:ok, result} <-
        ...
        if result do
          ...
          Logger.debug(
            "Transaction with hash #{pending_tx_hash_str} doesn't exist in the node anymore. We should remove it from Maalscan DB.",
            fetcher: :pending_transactions_to_refetch
          )
  ...
  defp fetch_pending_transaction_and_delete(transaction) do
    ...
    case transaction
      ...
      {:ok, _transaction} ->
        Logger.debug(
          "Transaction with hash #{pending_tx_hash_str} successfully deleted from Maalscan DB because it doesn't exist in the archive node anymore",
          fetcher: :pending_transactions_to_refetch
        )
      ...
      {:error, changeset} ->
        Logger.debug(
          [
            "Deletion of pending transaction with hash #{pending_tx_hash_str} from Maalscan DB failed",
            inspect(changeset)
          ],
          fetcher: :pending_transactions_to_refetch
        )
